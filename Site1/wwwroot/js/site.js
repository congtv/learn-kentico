...
(function (Site1) {
     ...
     */
    Site1.showMessage = function (message, type, logToConsole) {
        var messageElement = document.querySelector(".mc-system-messages");

        if (message && type) {
            if (type === "info") {
                messageElement.appendChild(Site1.buildMessageMarkup(message, "light-blue lighten-5"));

                if (logToConsole) {
                    console.info(message);
                }
            } else if (type === "warning") {
                messageElement.appendChild(Site1.buildMessageMarkup(message, "yellow lighten-4"));

                if (logToConsole) {
                    console.warn(message);
                }
            } else if (type === "error") {
                messageElement.appendChild(Site1.buildMessageMarkup(message, "deep-orange lighten-2"));
     ...
     */
    Site1.buildMessageMarkup = function (message, cssClasses) {
