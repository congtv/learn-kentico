(function () {
    window.kentico.pageBuilder.registerInlineEditor("slideshow-editor", {
        init: function (options) {
            ...
            var swiper = window.Site1
                .slideshowWidget
                .getCurrentSwiper(editor, window.Site1.slideshowWidget.swiperGuidAttribute);

            // Image GUID retrieval: Alternative 1 (begin)
            /*var slideIds = window.Site1.slideshowWidget.collectImageIds(swiper);

            var imageGuids = slideIds.map(function (slideId) {
                return window.Site1.slideshowWidget.getGuidFromId(slideId);
            });*/
            ...
            var addSlide = function () {
                ...
                var markup =
                    buildSlideMarkup(
                        tempGuid,
                        options.localizationService.getString("Site1.InlineEditor.Slideshow.PickImage"));
                ...
                pickingLink.addEventListener("click", function () {
                    if (!mediaLibraryName) {
                        showErrorMessage(slide, tempGuid,
                            options.localizationService.getString("Site1.NoLibrarySpecified"));
                    } else {
        ...
        destroy: function (options) {
            var swiper = window.Site1
                .slideshowWidget
                .getCurrentSwiper(options.editor, window.Site1.slideshowWidget.swiperGuidAttribute);

            if (swiper) {
                window.Site1.slideshowWidget.removeSwiper(swiper.el.id);
                swiper.destroy();
            }
        }
    });
